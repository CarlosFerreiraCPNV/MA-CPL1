...
def main():
    ...
    for x1, x2, y_true in DATA:
        z = W1 * x1 + W2 * x2 + B          # seulement pour l'affichage
        p = predict_proba(x1, x2)         # fonction réutilisée
        y_pred = predict(x1, x2)          # fonction réutilisée

        if y_pred == y_true:
            correct += 1

        print(f"{x1:.1f}\t{x2:.1f}\t{z:.4f}\t{p:.4f}\t{y_pred}\t{y_true}")
